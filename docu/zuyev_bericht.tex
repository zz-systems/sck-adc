...
\begin{document}
	...
	\section{Ergebnisse}
		...
		\begin{table}[h!]
			...
			\begin{tabular}{l r r}
				...
				V\textsubscript{os} 											& $ \approx \SI{10}{\micro\volt}  $ &  0.5--\SI{6}{\milli\volt}  \\			
	...
	\label{sec:appendix_measurement_methods}
	...
	\minisec{Slew rate}	
	\begin{enumerate}
		\item Cursors $\rightarrow$ Toggle On/Off
		\item Cursor \enquote{REF} per Edit Properties $\rightarrow$ X1 = 200n auf \SI{200}{\nano\second} setzen
		\item Cursor \enquote{A} auf \SI{400}{\nano\second} setzen
		\item Spannungsdifferenz mit 5 multiplizieren
	\end{enumerate}

	\minisec{Bias current}
	\begin{enumerate}
		\item Kurve \enquote{I(bias)} auswählen
		\item Measure $\rightarrow$ Mean $\Rightarrow I_{bias}$
	\end{enumerate}

	\minisec{CMRR, PSRR}
	Hier wurde ein kombiniertes Messverfahren \cite{website:amp_psrr_cmrr_circuits} für mehrere Parameter verwendet.
	\begin{enumerate}
		\item Die Kurven $V_{os PSRR+}$, $V_{os PSRR-}$, $V_{os CMRR}$, $V_{ddn}$, $V_{ssn}$, $V_{cmn}$ auswählen.
		\item Measure $\rightarrow$ More Functions... $\rightarrow$ Peak To Peak $\Rightarrow \hat{V}$, \emph{Amplitude}
		\item $ PSRR+ = 20 * log_{10} \frac{\hat{V}_{ddn}}{\hat{V}_{os PSRR+}}$
		\item $ PSRR- = 20 * log_{10} \frac{\hat{V}_{ssn}}{\hat{V}_{os PSRR-}}$
		\item $ CMRR = 20 * log_{10} \frac{\hat{V}_{cmn}}{\hat{V}_{os CMRR}}$
	\end{enumerate}

	\minisec{Gain bandwidth product}
	\begin{enumerate}
		\item Die Kurve $dB$ auswählen
